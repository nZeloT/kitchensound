...
Renderer::Renderer()
{
    if (SDL_Init(SDL_INIT_VIDEO) < 0) {
        spdlog::error("SDL_Init(): {1}", SDL_GetError());
        throw std::runtime_error("Error initializing SDL!");
    }
    spdlog::info("SDL_VIDEODRIVER selected : {1}", SDL_GetCurrentVideoDriver());
    ...
    if (nullptr == window) {
        spdlog::error("SDL_CreateWindow(): {1}", SDL_GetError());
        throw std::runtime_error("Error creating SDL Window");
    }

    renderer = SDL_CreateRenderer(window, -1, SDL_RENDERER_ACCELERATED);
    if (nullptr == renderer) {
        spdlog::error("SDL_CreateRenderer(): {1}", SDL_GetError());
        throw std::runtime_error("Error creating SDL renderer");
    }
    SDL_RendererInfo info;
    SDL_GetRendererInfo(renderer, &info);
    spdlog::info("SDL_RENDER_DRIVER selected : {1}", info.name);
...
void Renderer::init_sdl_ttf() {
    if (TTF_Init() < 0) {
        spdlog::error("TTF_init(): {1}", TTF_GetError());
        throw std::runtime_error("Error loading SDL ttf");
    }
    SDL_version version{};
    SDL_TTF_VERSION(&version);
    spdlog::info("Using SDL_TTF_Version: {1}.{2}.{3}",
...
void Renderer::init_sdl_image() {
    ...
    if ((IMG_Init(initflags) & initflags) != initflags) {
        spdlog::error("IMG_Init(): {1}", IMG_GetError());
        throw std::runtime_error("Error loading SDL img");
    }
    SDL_version version{};
    SDL_IMAGE_VERSION(&version);
    spdlog::info("Using SDL_IMG_Version: {1}.{2}.{3}",
                 std::to_string(version.major),
                 std::to_string(version.minor),
                 std::to_string(version.patch));
}
